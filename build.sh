...
cp "$LIB_KITSUNE_COMMON_DIR/src/libKitsuneCommon.so.0.5.4" "$RESULT_DIR/"
cp -r "$DIR/include" "$RESULT_DIR/"

# recreate symlinks
cd "$RESULT_DIR/"
ln -s libKitsuneCommon.so.0.5.4 libKitsuneCommon.so.0.5
ln -s libKitsuneCommon.so.0.5.4 libKitsuneCommon.so.0
ln -s libKitsuneCommon.so.0.5.4 libKitsuneCommon.so
